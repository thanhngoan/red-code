...
public class Sudoku {
	...
	public static final int[][] hardGridMod = Sudoku.stringsToGrid(
			"3 0 0 0 0 0 0 8 0",
			"0 0 1 0 9 3 0 0 0",
			"0 4 0 7 8 0 0 0 3",
			"0 9 3 8 0 0 0 1 2",
	...
	{
		List<Spot> thespots = new LinkedList<Spot>();
		
		for (int i=0; i < SIZE; i++)	
			for (int j=0; j < SIZE; j++)
				thespots.add(new Spot(i+1, j+1));
		
		Collections.sort(thespots, new Comparator<Spot>() {
			public int compare(Spot s1, Spot s2) {
				return s1.possibleNumbers().size() - s2.possibleNumbers().size();	
			}
		});
		
		Stack<Spot> spotstack = new Stack<Spot>();
